...
public class AcquisitionPlanService {
    ...
    public Summary summary(UUID id) {
        ...
        BigDecimal principalWithKnownRepayment = BigDecimal.ZERO;
        boolean totalsComplete = true;
        for (FinancialObligation obligation : linked) {
            BigDecimal knownTotal = knownRepaymentTotal(obligation);
            if (knownTotal == null) {
                totalsComplete = false;
                continue;
            }
            repayment = repayment.add(knownTotal);
            principalWithKnownRepayment = principalWithKnownRepayment.add(
        ...
        BigDecimal cost = repayment.subtract(principalWithKnownRepayment);
        return new Summary(
            plan,
            List.copyOf(linked),
    ...
    private BigDecimal knownRepaymentTotal(FinancialObligation obligation) {
        if (obligation.getPlannedTotalAmount() != null) {
            return obligation.getPlannedTotalAmount();
        }
        if (obligation.getCalculationMethod() == ObligationCalculationMethod.INTEREST_FREE) {
            return obligation.getPrincipalAmount();
        }
        return null;
    }
