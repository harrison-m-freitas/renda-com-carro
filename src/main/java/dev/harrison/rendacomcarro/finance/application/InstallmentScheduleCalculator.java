...
public class InstallmentScheduleCalculator {
    ...
    public RepaymentSchedule calculateInterestFree(
        ...
        long principalCents = normalizedPrincipal.movePointRight(2).longValueExact();
        if (principalCents < installments) {
            throw error(
                "termMonths",
        ...
        long baseCents = principalCents / installments;
        long remainderCents = principalCents % installments;
        List<ScheduleEntry> entries = new ArrayList<>(installments);
        for (int sequence = 1; sequence <= installments; sequence++) {
            boolean receivesRoundingCent = sequence > installments - remainderCents;
            long entryCents = baseCents + (receivesRoundingCent ? 1 : 0);
            BigDecimal amount = BigDecimal.valueOf(entryCents, 2);
        ...
        BigDecimal displayedInstallment = money(
            normalizedPrincipal.divide(BigDecimal.valueOf(installments), CALCULATION_CONTEXT)
        );
