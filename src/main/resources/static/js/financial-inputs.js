...
export function formatInput(input) {
  ...
  const canonical = input.dataset.financialCanonical === 'true';
  input.value = input.matches('[data-financial-percent]')
    ? (canonical ? formatCanonicalPercentage(input.value) : formatFinancialPercentage(input.value))
    : (canonical ? formatCanonicalMoney(input.value) : formatFinancialMoney(input.value));
  if (canonical) delete input.dataset.financialCanonical;
}
