...
function fixedPreview(input, principal) {
  ...
  if (input.calculationMethod === 'INSTALLMENT_KNOWN') {
    ...
    schedule = buildSchedule(principal, installmentAmount, monthlyRate, termMonths);
    ...
    monthlyRate = 0;
    const distribution = distributeInterestFreeCents(principal, termMonths);
    installmentAmount = distribution.installmentAmount;
    installmentAmounts = distribution.installmentAmounts;
    schedule = { totalAmount: distribution.totalAmount };
  } else {
...
function distributeInterestFreeCents(principal, termMonths) {
  const principalCents = Math.round(principal * 100);
  if (principalCents < termMonths) {
    throw calculationError(
      'termMonths',
      'A quantidade de parcelas exige valores menores que um centavo.',
    );
  }
  const baseCents = Math.floor(principalCents / termMonths);
  const remainderCents = principalCents % termMonths;
  const lower = baseCents / 100;
  const higher = (baseCents + (remainderCents > 0 ? 1 : 0)) / 100;
  return {
    installmentAmount: roundMoney(principalCents / 100 / termMonths),
    installmentAmounts: remainderCents > 0
      ? {
        higher,
        higherCount: remainderCents,
        lower,
        lowerCount: termMonths - remainderCents,
      }
      : null,
