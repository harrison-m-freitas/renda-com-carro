...
if (form) {
  ...
  function refreshSummary() {
    ...
    if (input.mode === 'FIXED_INSTALLMENTS') {
      setText(summary.payment, fixedPaymentText(input.termMonths, preview));
  ...
  document.addEventListener('guided-form:restored', (event) => {
    if (event.detail.form !== form) return;
    const restoredPayload = event.detail.draft?.payload ?? {};
    for (const name of [
      'principalAmount', 'interestRatePercent', 'installmentAmount',
      'singlePaymentAmount', 'monthlyTarget',
    ]) {
      const control = form.elements.namedItem(name);
      if (control && Object.hasOwn(restoredPayload, name)) {
        control.dataset.financialCanonical = 'true';
      }
    }
    initializeFinancialInputs(form);
    refreshConditionalPanels();
...
function fixedPaymentText(termMonths, preview) {
  const distribution = preview.installmentAmounts;
  if (!distribution) return `${termMonths} × ${money(preview.installmentAmount)}`;
  return `${termMonths} parcelas entre ${money(distribution.lower)} e ${money(distribution.higher)}`;
}
