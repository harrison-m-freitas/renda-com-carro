...
if (form) {
  ...
  function refreshSummary() {
    ...
    setText(
      ...
      formatMoney(parseLocalizedDecimal(principal?.value) ?? 0)
    );
    setText(
      '[data-obligation-summary-rate]',
      `${formatPercentage(parseLocalizedDecimal(annualRate?.value) ?? 0)}% ao ano`
    );
    const structured = mode?.value === 'STRUCTURED';
    const plan = structured
      ? `${termMonths?.value || 0} parcelas a partir de ${formatDate(firstDueDate?.value)}`
      : `${formatMoney(parseLocalizedDecimal(monthlyTarget?.value) ?? 0)} por mês`;
  ...
  [creditor, principal, annualRate, firstDueDate, termMonths, plannedInstallment, monthlyTarget]
