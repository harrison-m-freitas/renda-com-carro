...
class AcquisitionPlanWebTest extends PostgresIntegrationTest {
    ...
    @Test
    @WithMockUser(username = "acquisition-web-owner", roles = "OWNER")
    void detailLabelsUnknownRepaymentTotalsAsPartial() throws Exception {
        ...
        mvc.perform(get("/acquisition-plans/{id}", plan.getId()))
            .andExpect(status().isOk())
            .andExpect(content().string(containsString("Subtotal conhecido")))
            .andExpect(content().string(containsString("Existem obrigações sem total calculado")));
    }
