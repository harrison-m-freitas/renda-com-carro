...
test('finance redesign follows current Flyway versions and removes incompatible obligation drafts', () => {
  ...
  assert.ok(files.includes('V13__redesign_financial_obligations.sql'));
  assert.equal(files.includes('V11__redesign_financial_obligations.sql'), false);

  const financeMigration = readFileSync(
    resolve(migrationDirectory, 'V13__redesign_financial_obligations.sql'),
    'utf8',
  );
